...
const RL32 = () => {
    ...
    const getRumahSakit = async (kabKotaId) => {
        try {
            const response = await axiosJWT.get('/apisirs/rumahsakit/', {
                headers: {
    ...
    const deleteRL = async (id) => {
        ...
        try {
            await axiosJWT.delete(`/apisirs/rltigatitikdua/${id}`,
                customConfig)
    ...
    const getProvinsi = async() => {
        try {
            ...
            const results = await axiosJWT.get('/apisirs/provinsi',
                customConfig)
    ...
    const getKabKota = async(provinsiId) => {
        try {
            ...
            const results = await axiosJWT.get('/apisirs/kabkota',
                customConfig)
                                            ...
                                            {/* <RiDeleteBin5Fill  size={20} onClick={(e) => hapus(value.id)} style={{color: "gray", cursor: "pointer", marginRight: "5px"}} /> */}
                                            {
                                                user.jenisUserId === 4 ? (
                                                    <div style={{ display: "flex" }}>
                                                        <button className="btn btn-danger" style={{ margin: "0 5px 0 0", backgroundColor: "#FF6663", border: "1px solid #FF6663" }} type='button' onClick={(e) => deleteConfirmation(value.id)}>H</button>
